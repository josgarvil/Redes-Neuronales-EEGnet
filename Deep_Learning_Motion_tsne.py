...
def traininig_generic_model(training_data, val_data, labels_training, labels_val, nb_classes, Chans, Samples, dropoutRate, kernLength, learning_rate, batch_size,epochs,path_model, earlystopping_patience):
    ...
    savemat('metrics_results_motion.mat',  {'Loss': val_loss, 'Accuracy': val_acc, 'F1score': val_f1},  oned_as='column' )

    return model


# Divide data in training and validation (70% training and 30% validation)
# x_train, x_val, y_train, y_val = train_test_split(training_motion_data, labels_motion_data_original.transpose(), test_size=def_test_size, random_state=seed_value, stratify=labels_motion_data_original.transpose())
x_train=copy.deepcopy(training_motion_data)
x_train=np.delete(x_train, np.s_[0:trial_epochs], axis=0)
x_val=training_motion_data[0:trial_epochs,:,:]

y_train=copy.deepcopy(labels_motion_data_original.transpose())
y_train=np.delete(y_train, np.s_[0:trial_epochs], axis=0)
y_val=labels_motion_data_original.transpose()[0:trial_epochs]

# One-hot encoder, transfor labels from 0 1 to [1 0] and [0 1]
y_train = oh.fit_transform(y_train).transpose()
y_val = oh.fit_transform(y_val).transpose()

net = traininig_generic_model(x_train, x_val, y_train.transpose(), y_val.transpose(), number_classes, training_motion_data.shape[1],
                                training_motion_data.shape[2], def_dropout_rate, def_kernLength,def_lr, def_batch_size, def_epochs,
                                'checkpoint_motion.h5', def_early_stopping_patience)
