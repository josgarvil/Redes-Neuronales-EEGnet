...
def traininig_generic_model(training_data, labels_training, nb_classes, Chans, Samples, dropoutRate, kernLength, learning_rate, batch_size,epochs,path_model, earlystopping_patience):
    ...
    savemat('metrics_results_motion.mat',  {'Loss': loss, 'Accuracy': acc, 'F1score': f1},  oned_as='column' )
...
x_train = training_motion_data
y_train = labels_motion_data_original.transpose()

# One-hot encoder, transfor labels from 0 1 to [1 0] and [0 1]
y_train = oh.fit_transform(y_train).transpose()
# y_val = oh.fit_transform(y_val).transpose()

traininig_generic_model(x_train, y_train.transpose(),number_classes, training_motion_data.shape[1],
                                training_motion_data.shape[2], def_dropout_rate,def_kernLength,def_lr, def_batch_size, def_epochs,
                                'checkpoint_motion_pretrained.h5', def_early_stopping_patience)
