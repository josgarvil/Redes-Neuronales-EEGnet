...
for i in range(7):  # int(length_data/trial_epochs)):
    ...
    x_val = training_static_data[trials_prev*trial_epochs+i*trial_epochs:trials_prev*trial_epochs+i*trial_epochs+trial_epochs,:,:]
    ...
    y_val = labels_static_data_original.transpose()[trials_prev*trial_epochs+i*trial_epochs:trials_prev*trial_epochs+i*trial_epochs+trial_epochs]
    ...
    checkpointer = ModelCheckpoint(filepath='checkpoint_static.h5', verbose=1)
    ...
    fittedModelprev = train_model.fit(x_train, y_train.transpose(), batch_size=def_batch_size, epochs=def_epochs,
                                      verbose=2, callbacks=[checkpointer, early_stopping, lr_scheduler], shuffle=True)

    # Clone model
    train_modelFT = load_model('checkpoint_static.h5', custom_objects={"recall_metric": recall_metric, "precision_metric": precision_metric, "f1_metric": f1_metric})
...
savemat('metrics_results_deep_learning_FT_static.mat', {'Loss': metric_value[::3], 'Accuracy': metric_value[1::3], 'Accuracy2': acc2, 'F1score': metric_value[2::3]},  oned_as='column')
